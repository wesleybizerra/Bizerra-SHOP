...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleCheckout = async () => {
    ...
    setIsLoading(true);
    try {
      console.log('V6-FINAL - Iniciando checkout...');
      ...
      if (data.init_point) {
        ...
        alert('V6-FINAL - Erro: O Mercado Pago não retornou o link de pagamento.');
      }
    } catch (error: any) {
      console.error('V6-FINAL - Erro de Checkout:', error);
      alert(`V6-FINAL - Erro: ${error.message}. Verifique se o Token do Mercado Pago no Netlify está correto e comece com APP_USR-.`);
    } finally {
      setIsLoading(false);
    }
  };
